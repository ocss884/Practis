...
def compute_conditional_rmse(
    targets: Iterable[pd.DataFrame], forecasts: Iterable[Forecast],
) -> np.float32:
    """
    Compute the non-normalized energy score for a multivariate stochastic prediction from samples.
    ...
    for target, forecast in zip(targets, forecasts):
        ...
        scale = 1.0#np.mean(target_data, axis=-1, keepdims=True)
        mse = np.mean(((mean - target_data)/scale)**2)


        cumulative_score += mse
        num_forecasts += 1
    return (cumulative_score / num_forecasts)**0.5

def compute_conditional_nrmse(
    targets: Iterable[pd.DataFrame], forecasts: Iterable[Forecast],
) -> np.float32:
    """
    Compute the non-normalized energy score for a multivariate stochastic prediction from samples.
    ...
    for target, forecast in zip(targets, forecasts):
        ...
        samples = forecast.samples
        mean = np.mean(samples, axis=0)
        scale = np.std(target_data, axis=-1, keepdims=True)
        mse = np.mean(((mean - target_data)/scale)**2)
...
def compute_validation_metrics(
    ...
    return {
        ...
        "Conditional-RMSE": compute_conditional_rmse(targets, forecasts),
        "Conditional-NRMSE": compute_conditional_nrmse(targets, forecasts),
        "num_nan": num_nan,
        "num_inf": num_inf,
    }
