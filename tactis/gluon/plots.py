...
def plot_single_forecast(
    ...
    print(forecast.prediction_length, len(target))
    ...
    forecast_index = [x.start_time for x in forecast_index]
    ...
    times = [time.start_time.asm8 for time in forecast.index]
    xmin = times[0] - (times[1] - times[0]) / 2#times[0] - (times[1] - times[0]) / 2
    xmax = times[-1] + (times[-1] - times[0])  # Beyond the limit of the plot
    ...
    #axes.set_xlim(xmin=times[-97], xmax=times[-1])
    print(len(times))
    #axes.set_xlim(xmin=times[0], xmax=times[-1])
...
def plot_four_forecasts(
    ...
    if savefile:
        fig.savefig(savefile, bbox_inches="tight", pad_inches=0, dpi=400)
