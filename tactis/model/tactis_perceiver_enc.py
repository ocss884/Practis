...
class TACTiS_PEncoder(nn.Module):
    """
    ...
    def __init__(
        ...
        for i in range(self.input_encoder_layers):
            if i == 0:
                elayers.append(
                    nn.Linear(self.series_embedding_dim + 2 + 1, self.encoder.embedding_dim)
    ...
    def loss(
        ...
        query_encoded = self.input_encoder(query_encoded)
        ...
        loss = self.decoder.loss(query_encoded, mask, true_value)#self.decoder.loss(query_encoded, mask, true_value, query_timesteps)
    ...
    def sample(
        ...
        samples = self.decoder.sample(num_samples, query_encoded, mask, true_value)#self.decoder.sample(num_samples, query_encoded, mask, true_value, query_timesteps)
